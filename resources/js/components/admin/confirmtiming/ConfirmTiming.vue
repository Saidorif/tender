		  		...
		  		<div class="header_title mb-2">
	            	...
	            	<div class="add_user_btn">
		                <span class="alert alert-info" style="    margin: 0px 15px 0px auto;">
		            		{{$t('Yoʼnalish soni')}} <b>{{ getTimings.total }} {{$t('ta')}}.</b>
		            	</span>
				...
				<table class="table table-bordered text-center table-hover table-striped">
					...
					<tbody>
						<tr v-for="(direct,index) in getTimings.data">
							...
							<td>{{direct.year}}</td>
                            <td>
                                <div class="badge" :class="getStatusClass(direct.xronom_status)">
	                                {{direct.xronom_status == 'completed' ? $t('Tasdiqlangan') : $t('Tasdiqlanmagan')}}
                                </div>
                            </td>
							<td>
								<router-link
