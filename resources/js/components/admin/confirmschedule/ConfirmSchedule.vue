		...
		<div class="card">
			  ...
			  <div class="table-responsive">
				<table class="table table-bordered text-center table-hover table-striped">
					...
					<tbody>
						<tr v-for="(direct,index) in getSchedules.data">
							...
							<td>
								<router-link
									...
									:to='`/crm/confirm-schedule/edit/${direct.id}`'
								>
					...
					<pagination :limit="4" :data="getSchedules" @pagination-change-page="getResults"></pagination>
				</table>
			  </div>
		  </div>
	  	</div>
...
<script>
	...
	export default{
		...
		async mounted(){
            let page = 1;
            await this.actionSchedules();
            this.laoding = false
		},
		computed:{
			...mapGetters('confirmschedule',['getSchedules', 'getScheduleMassage'])
		},
		methods:{
            ...mapActions('confirmschedule',['actionSchedules','actionActivateSchedule']),
			async getResults(page = 1){
				await this.actionSchedules(page)
			},
