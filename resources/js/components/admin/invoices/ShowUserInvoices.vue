	...
	<div class="payment">
		...
		<div class="card">
		  	<div class="card-header">
                ...
                <router-link class="btn btn-primary back_btn" to="/crm/userinvoices">
                ...
                <div class="card-body">
                    ...
                    <div class="invoice_blank_header" style="width: 100%;display: flex;justify-content: space-between;align-items: flex-start;margin-bottom: 30px;">
                        ...
                        <div class="customer" v-if="getShowUserInvoice.user" style="width: 45%;" >
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                {{ getShowUserInvoice.user.company_name}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('Manzil')}}:</b> {{getShowUserInvoice.user.address}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('Telefon')}}:</b>{{ getShowUserInvoice.user.phone}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('X/raqam')}}:</b>{{ getShowUserInvoice.user.bank_number}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('Shaxar')}}:</b> {{getShowUserInvoice.user.city}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('INN')}}:</b> {{getShowUserInvoice.user.inn}}
                                <b class="provider_text_title">{{$t('MFO')}}:</b> {{getShowUserInvoice.user.mfo}}
                            </p>
                            <p class="provider_text" style="margin-bottom: 0;font-size: 14px;font-style: italic;">
                                <b class="provider_text_title">{{$t('OKED')}}:</b>{{ getShowUserInvoice.user.oked}}
                            </p>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover text-center table-bordered" style="font-size: 14px;border: 2px solid black; ">
                            ...
                            <tbody>
                                <tr >
                                    ...
                                    <td scope="col" style="padding: 0.25rem !important;vertical-align: middle important;font-size: 12px important;border: solid #000 !important;border-width: 0 1px 1px 0 !important;">{{getShowUserInvoice.price}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script>
	...
	export default{
		...
		async mounted(){
			await this.actionShowUserInvoice(this.$route.params.id)
            await this.actionSetting()
			this.laoding = false
		},
		computed:{
			...mapGetters('invoices',['getShowUserInvoice']),
            ...mapGetters('setting',['getSetting'])
		},
		methods:{
            ...mapActions('invoices', ['actionShowUserInvoice']),
