  ...
  <div class="add_area">
    ...
    <div class="card card_with_tabs">
      ...
      <div class="card-body">
        ...
        <div
          class="tarif_column"
          v-if="titulData.type && titulData.type.id != 1"
        >
        ...
        <div id="prindDiv">
          ...
          <p style="margin: 0px; text-align: right; font-size: 15px">Сана</p>
            <h1 style="margin: 0px;text-align: center;font-size: 19px;font-weight: bold;margin-bottom:10px;">
              Йўл кира ҳақи жадвали {{ titulData.pass_number }} - сонли "{{
                titulData.name
              }}"
            </h1>
            <div class="table-responsive" v-for="(t_item, t_index) in getTarif" v-if="t_item.tarif.status == 'approved'">
              ...
              <table style="border-collapse: collapse;margin-bottom:20px;width:100%;">
                <thead>
                  <tr>
                    <th  style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;">№ т/р</th>
                    <th  style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;">Бошлангич ва оралик охирги бекатлар номи</th>
                    <template
                      :colspan="titulData.timing_with.length"
                      v-for="(item, index) in titulData.timing_with"
                    >
                      <th  style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;">
                        {{ item.whereTo.name }}
                      </th>
                    </template>
                  </tr>
                ...
                <tbody>
                  <tr v-for="(items, index) in t_item.items">
                    <td style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;">{{ index + 1 }}</td>
                    <td style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;">
                      {{ items[index].from_name ? items[index].from_name : "" }}
                    </td>
                    <template v-for="(item, key) in items">
                      <td
                      style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;"
                        v-if="item.tarif"
                        :style="[key == items.length - 1 && index == 0 ? { backgroundColor: '#f9d6d5'} : { backgroundColor: '#fff'}]"
                        :class="
                          key == items.length - 1 && index == 0
                            ? 'alert-danger'
                            : ''
                      ...
                      >
                        <div class="tarifs tarif">
                          <b style="color: #a81a00;">{{ item.tarif }}</b>
                        </div>
                        <div class="tarifs tarif_bagaj">
                          <b style="color: #324841;">{{ item.tarif_bagaj }}</b>
                        </div>
                        <div  class="tarifs tarif_bagaj">
                          <b style="color: #324841;">{{ item.distance_test }}KM</b>
                        </div>
                      </td>
                      <template v-else>
                        <td style="padding: 0.25rem;font-size:13px;border: 1px solid #292666;text-align:center;background: #9a9a9a;" class="has_no_name_tarif" v-if="key != 'ddd'"></td>
                      </template>
                    </template>
                  </tr>
                </tbody>
